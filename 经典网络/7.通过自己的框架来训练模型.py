...
def train_my_model(dataset_name,transform,batch_size,net_model,learning_rate,epoch,model_name):
    ...
    # 训练集
    train_data = dataset_class(root = '../数据集/{}'.format(dataset_name.lower()),train=True,transform=transform,download=True)
    # 测试集
    test_data = dataset_class(root = '../数据集/{}'.format(dataset_name.lower()),train=False,transform=transform,download=True)
    ...
    for i in range(train_epoch):
        ...
        for data in train_dataloader:
            ...
            if total_train_step % 100 == 0:
                print("训练次数：{}|训练样本数：{}|单次训练误差(loss):{}".format(total_train_step,total_train_data,loss.item()))
        # 计算一轮训练的平均loss
