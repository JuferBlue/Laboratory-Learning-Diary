...
class ASKCResNetFPN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.stem(x)          # 1 3 64 64
        c1 = self.layer1(x)       # 1 16 64 64
        c2 = self.layer2(c1)      # 1 32 32 32
        out = self.layer3(c2)     # 1 64 16 16

        out = F.interpolate(out, size=[hei//8, wid//8], mode='bilinear')  # 1 64 32 32
        out = self.fuse23(out, c2)                                        # 1 32 32 32

        out = F.interpolate(out, size=[hei//4, wid//4], mode='bilinear')  # 1 32 64 64
        out = self.fuse12(out, c1)                                        # 1 16 64 64

        pred = self.head(out)                                             # 1 1 64 64
        out = F.interpolate(pred, size=[hei, wid], mode='bilinear')       # 1 1 256 256
...
class ASKCResUNet(nn.Module):
    def __init__(self, layer_blocks, channels, fuse_mode='BiLocal'):
    ...
    def forward(self, x):
        _, _, hei, wid = x.shape   #(2,3,256,256)

        x = self.stem(x)           #(2,3,256,256)
        c1 = self.layer1(x)        #(2,16,64,64)
        c2 = self.layer2(c1)       #(2,32,32,32)
        c3 = self.layer3(c2)       #(2,64,16,16)

        deconc2 = self.deconv2(c3) #(2,32,32,32)
...
if __name__ == "__main__":
    print('-----'*5)
    rgb = torch.randn(1, 1, 200, 200)
    net = ASKCResUNet([4] * 3, [8, 16, 32, 64])
    out = net(rgb)
    print(out.shape)
